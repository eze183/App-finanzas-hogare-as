...
### 2026-07-29 (render arquitectonico fotorealista)

- Se creo `EXTERIOR_ATARDECER_03_fotorealista.png` como version nueva del render exterior entregado por el usuario. Se conservaron encuadre, arquitectura, proporciones y materiales; solo se aumentaron la fidelidad fotografica de la luz, los ladrillos, revoques, madera, vidrio, vegetacion y sombras. El JPG original fuera de este proyecto no se modifico.
- Se creo `EXTERIOR_ATARDECER_03_perspectiva_referencia.png` tomando una foto de obra como referencia estricta de camara, encuadre y perspectiva. Se amplio el campo de piso y la lectura longitudinal del pasillo, conservando el proyecto terminado (puerta, muro verde, ladrillo, revoque, nicho y materiales) sin usar los acabados deteriorados de la foto de obra.
- Se creo `WhatsApp Image 2021-12-28_enderezada.png` mediante una rotacion geometrica de 1,7 grados para nivelar una foto de fachada, sin retoques de contenido. La minima zona adicional de las esquinas queda transparente por la rotacion para no inventar o modificar pixeles de la imagen original.
...
- Causa: `detectDocumentType` en `app.js` contaba como "linea con fecha" cualquier texto que tuviera un numero-separador-numero en cualquier parte de la linea (`/\d{1,2}[\/.-]\d{1,2}/`). Un ticket real esta lleno de falsos positivos con ese patron que no son fechas: el CUIT (`27-20195663-9`), el numero de comprobante (`00017-00086901`), el IVA contenido (`8,677.69`), y sobre todo las lineas de productos pesados como `2.000 x $ 7850.00` o `0.614 x $ 7990.00`. Alcanzando 3 "fechas" falsas, la app clasificaba el ticket como resumen de tarjeta y lo mandaba por el camino de extraccion equivocado (tabla fecha-concepto-cupon-monto), que no tiene nada que ver con un ticket, resultando en 0 consumos encontrados.
- Fix: se reutilizo `matchStatementDate` (la logica de fecha estricta, anclada al inicio de linea, ya usada para el resumen de Banco Nacion) y se agrego `resolveStatementDate`, que ademas valida que el dia/mes/año formen una fecha real (rechaza cosas como "2.000" -> dia 2, mes 0, invalido). Con esto, los conteos de "lineas con fecha" en un ticket real bajan a 0-1 en vez de 5+, y ambos tickets de prueba ahora se clasifican correctamente como "receipt". Tambien se acotaron las palabras clave de "es un resumen" a frases especificas de resumen de tarjeta ("resumen consolidado", "cuotas del mes", "saldo actual", etc.) en vez de palabras sueltas como "tarjeta" o "comprobante" que tambien pueden aparecer en un ticket comun pagado con tarjeta.
- Verificado simulando el parser completo con el texto real de ambos tickets: los dos se clasifican como "receipt" y extraen correctamente monto y fecha (Granjas Pampeanas: $50.000, 11/07/2026; La Anonima: $56.206,39, 11-07-26, tomando el TOTAL y no un precio suelto de la lista de productos).
- Segundo bug encontrado probando el ticket de Granjas Pampeanas a traves del flujo real de la app (inyectando un archivo .txt con el texto exacto en el input de archivo, sin poder probar el OCR real porque las fotos se pegaron en el chat y no hay acceso a esos archivos en disco): `extractReceiptDate` tomaba la PRIMERA fecha de todo el texto sin importar el contexto, y el ticket tiene "Inicio de Actividades: 01/05/2003" antes que la fecha real "Fecha 11/07/2026" — cargaba el gasto con fecha 2003 en vez de 2026. Fix: `extractReceiptDate` ahora prioriza lineas que contienen la palabra "fecha" y descarta lineas con "inicio"/"actividad"/"vencimiento"/"nacimiento", con el mismo patron de prioridad que ya se usaba para el monto. Verificado con ambos tickets reales a traves del flujo completo de la app: fecha, monto y mensaje de exito correctos en los dos.
