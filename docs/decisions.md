...
> **Actualización 2026-09-15 (`codex/proteccion-datos`)**: esta sección conserva la decisión histórica, pero ya no describe el lector vigente. Los cierres dejaron de deduplicarse por `weekKey`: cada actualización crea un registro inmutable con `supersedes` y un `adjustment` que representa solo la diferencia todavía pendiente. `getActiveSettlements()` resuelve la versión activa de cada cadena; historial muestra cierres anteriores y reaperturas. Los períodos superpuestos conocidos se bloquean, y los conflictos creados offline se conservan para conciliación explícita. Ver `docs/proteccion-datos.md`.
...
**Decisión** (2026-07-20): para el problema de "compro algo en cuotas con una de las 4 tarjetas y me olvido de que la sigo pagando", se evaluó crear una entidad `installmentPurchases` separada (con generación automática de un gasto por mes) pero se descartó a pedido explícito del usuario ("me gustaría que sea simple"). En cambio: `personalExpenses` ganó dos campos opcionales, `card` (una lista fija de tarjetas, definida solo como `<option>` en `index.html`; al 2026-08-26 son seis, con las de Banco Pampa agregadas — no hay constante ni validación en `app.js`, se guarda el string tal cual) y `installments` (cantidad de cuotas, 1 por defecto). El monto cargado sigue siendo el total de la compra, una sola vez.
...
- La tira recordatoria **solo se muestra en modo Personales** (`currentEntryMode === "personal"`, además de estar en Cargar o Resumen). En la versión original (2026-08-06) se mostraba también en Comunes a propósito ("que no se olvide"), pero el usuario reportó el 2026-08-07 que un gasto de tarjeta cargado en Personales "se veía también en Comunes" — era esta tira, no el gasto en sí (los datos de `expenses` y `personalExpenses` siempre estuvieron bien separados). Confundía más de lo que ayudaba: mezclar información personal en el contexto de gastos compartidos. Corregido gateando también por `currentEntryMode` en `renderInstallmentsReminder()`.
