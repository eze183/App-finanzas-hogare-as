...
- **La foto/OCR no autocompletaba el formulario personal**: hizo falta arreglarlo en dos pasos. Primero, `fillExpenseFromReceipt()` escribía siempre en los campos del formulario común, así que en modo personal el monto quedaba vacío; ahora respeta el modo activo y completa el dueño si estaba vacío. Pero eso **no alcanzó**, porque la app arranca en Comunes y el flujo natural es sacar la foto y *después* decidir que el gasto es personal: al cambiar de pestaña el formulario seguía vacío. Se agregó entonces que **el gasto en progreso se traslade de formulario al cambiar de pestaña** (se mueve, no se copia). Verificado con la factura real del usuario, corriendo el OCR de Tesseract sobre su propia foto.
