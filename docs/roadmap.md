...
Estado real al 2026-08-27, extraído del código, del historial de git y de la última sesión. Se actualiza automáticamente al cerrar cada funcionalidad importante.
...
## Dictado por voz: forma de pago y concepto separados — completo, 2026-08-27

"Netflix tarjeta de crédito" ahora completa forma de pago y categoría por separado en vez de amontonar todo en la descripción. Se agregó `detectVoicePaymentMethod()` y se corrigió un bug de substring (`text.includes`) que categorizaba mal por palabras contenidas dentro de otras (ej. "gas" dentro de "gasté"). Detalle en `session-summary.md`.
...
- **La foto/OCR no autocompletaba el formulario personal**: hicieron falta tres pasos, cada uno descubierto porque el anterior no alcanzaba. (1) `fillExpenseFromReceipt()` escribía siempre en los campos del formulario común; ahora respeta el modo activo. (2) La app arranca en Comunes y el flujo natural es sacar la foto y *después* decidir que es personal, así que el gasto en progreso ahora se traslada de formulario al cambiar de pestaña (se mueve, no se copia). (3) Con el traslado ya andando, el celular real seguía sin extraer el monto (`$88.00`, sin relación con el ticket) porque la imagen de la cámara se le pasaba a Tesseract sin ningún preprocesamiento — resolución muy alta, orientación EXIF sin aplicar, contraste disparejo. Se agregó `prepareImageForOcr()`: reorienta, redimensiona a 2000px de lado largo y sube contraste en escala de grises antes de pasarle la imagen a Tesseract. **Confirmado por el usuario en el celular real.**
